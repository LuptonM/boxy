...
export function isIgnored(selector: string, ignoreList: string[]): boolean {
  const normalized = stripOccurrence(selector);
  return ignoreList.some(pattern => {
    ...
    // Backward-compatible shorthand for entries like "dropdown".
    // CSS-looking entries are treated as selector fragments, not arbitrary substrings.
    if (!/[#.[>:]/.test(p)) {
...
function stripOccurrence(selector: string): string {
...
export function checkClipping(el: ElementModel): Issue | null {
  ...
  // Positioned elements clipped by a parent are usually bugs: dropdowns,
  // popovers, tooltips, sticky/floating headers, and transformed menus.
  ...
  if (!['absolute', 'fixed', 'sticky'].includes(el.position) && !isTransformed(el)) return null;
...
function isTransformed(el: ElementModel): boolean {
